...
class ComponentFive extends JComponent{ // формирования компонента для отрисовки
...
class ShapeMod{ // оболочка для хранения фигуры по более общему интерфейсу Shape
...
class RectangleMod extends Rectangle2D.Double{ // оболочка для хранения Rectangle2D.Double вместе с цветом
...
class EllipsMod extends Ellipse2D.Double{ // оболочка для хранения Ellipse2D.Double вместе с цветом
    private Color color;
...
class MouseClicker extends MouseAdapter{ // класс перехватчика событий мыши на основе адаптера
